...
// Remove the desktop Support CTA button while keeping the nav container intact.
output = output.replace(
  /\n\s*\{\/\* Support CTA Button \*\/\}[\s\S]*?<\/div>\n\n\s*\{\/\* Hamburger Menu Icon \(Mobile\) \*\//,
  "\n\n          {/* Hamburger Menu Icon (Mobile) */"
);

// Remove the mobile Support Dev button from the full-screen drawer.
output = output.replace(
  /\n\s*<motion\.a\n\s*variants=\{\{\n\s*open: \{ opacity: 1, y: 0 \},\n\s*closed: \{ opacity: 0, y: 20 \},\n\s*\}\}\n\s*href="https:\/\/ezdn\.app\/blue_tcc"[\s\S]*?\n\s*<\/motion\.a>/,
  ""
);

// Change the hero CTA from Explore Products to Donate.
...
// Keep the logo on the left and center the desktop navigation independently
// of the logo width or the removed Support button.
output = output.replace(
